...
function makeClock(
  ...
  const svg = clockInner.append('svg').attr('width', w).attr('height', h).attr('class', 'clock');
  ...
  function updateDigital(el, r, angle) {
    if (digitalCircle) {
      ...
      A ${r} ${r} 0 ${largeArcFlag} 1 ${xOffset(r * Math.sin(angle)) - 0.1} ${yOffset(
        r * Math.cos(angle)
      )}
      L ${xOffset(rEff * Math.sin(angle))}, ${yOffset(rEff * Math.cos(angle))}
      A ${rEff} ${rEff} 0 ${largeArcFlag} 0 ${xOffset(0) + 0.1}, ${yOffset(rEff)}
      Z`;
  ...
  function makeDigital() {
    if (digitalCircle) {
      ...
      L ${xOffset(rEff * Math.sin(2 * Math.PI))}, ${yOffset(rEff * Math.cos(2 * Math.PI))}
      ...
      M ${xOffset(0)}, ${yOffset(rEff)}
      L ${xOffset(0)}, ${yOffset(r)}
      A ${r} ${r} 0 ${largeArcFlag} 1 ${xOffset(r * Math.sin(endAngle))} ${yOffset(
        r * Math.cos(endAngle)
      )}
      L ${xOffset(rEff * Math.sin(endAngle))}, ${yOffset(rEff * Math.cos(endAngle))}
  ...
  function makeNeedle() {
    ...
    if (needleArc) {
      g.append('path')
        ...
        .attr(
          'd',
          `M ${w / 2} ${h / 2} L ${xOffset(rEff * Math.sin(endAngle))} ${yOffset(
